...
export default function PdfImport(){
 ...
 const needBatch=!batch&&files.length>0;
 const blocker=!cap.data?(cap.error?'Import limits could not be loaded — refresh above.':'Loading import limits…'):batches.error?'Batch list could not be loaded — refresh below.':!batch?(list?.length?'Step 1: choose a batch in the “Batch” box above (or create one) — the upload button unlocks as soon as a batch is selected.':'No batches exist yet — type a name below and click “Create & select batch” to unlock the upload button.'):!files.length?'Choose one or more PDFs to enable “Upload & analyze”.':problems[0]||'';
