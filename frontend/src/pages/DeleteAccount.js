...
export default function DeleteAccount() {
  ...
  return <div className="min-h-screen flex flex-col"><PublicHeader/><main className="w-full max-w-xl mx-auto p-6 flex-1 space-y-5">
    ...
    {result?<section data-testid="deletion-result" className="space-y-3"><h2>Account access removed</h2><p>Reference: {result.reference}</p><p>{result.status}: {result.detail}</p><p>External erasure is not complete.</p></section>:<form onSubmit={submit} className="space-y-4">
      ...
      <Button disabled={busy||(!!challenge&&!confirm)} variant="destructive" data-testid="delete-submit-button">{busy?'Please wait…':challenge?'Verify & Delete Account':'Send deletion OTP'}</Button>
