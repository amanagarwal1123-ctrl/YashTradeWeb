...
- Phase 10 (2026-09-10, build 2026.09.10-login-v10) — LOGIN "Something went wrong" + AGGRESSIVE LOAD TEST — COMPLETED (testing agent iteration_6: 16/16 pass):
  - ROOT CAUSE (login): old build called the API on the baked-in REACT_APP_BACKEND_URL host while the page was served from another host (yash-register.emergent.host / register.yashsilver.com) → cross-origin + `allow_origins=*` with credentials → browser blocked → old errMsg fallback "Something went wrong". Even when CORS passed, the SameSite=lax cookie would not have been sent cross-site.
  - FIX (already published as v9, verified in real browser on BOTH hosts): frontend calls same-origin (src/lib/backendUrl.js); CORS echoes exact origin via allow_origin_regex; errMsg distinguishes network / timeout / 4xx / 5xx and now appends HTTP status to the generic fallback; Cloudflare 429 HTML → "security layer is checking your connection".
  - SECOND LATENT LOGIN RISK FIXED (v10): admin verify-otp awaited get_customer_token() on the Yash Trade App backend (3 retries × 25s timeout; with demo mode off it fires a stray SMS to the admin and always fails) → 2s per login, up to ~80s if the app host is slow. Now: session + cookie returned immediately (0.08s); live-modules unlock runs as a background task with a 12s hard cap (env LIVE_ADMIN_UNLOCK=true/false, LIVE_ADMIN_UNLOCK_TIMEOUT), /admin/auth/me exposes live_unlock_pending, AdminLayout polls /me up to 4x while pending.
  - LOAD TEST TRUTH: the "~120 RPS bottleneck" was the Python httpx load generator, NOT the server. ApacheBench, single uvicorn worker: public/config 4,361 RPS; health 2,191; 422 validation 2,863; enroll/verify-otp (DB) 2,046; admin send-otp unauthorized (lockout query) 1,830; admin/me (session read+write) 1,486; admin/customers 707; sms/logs 724; audit-logs 615 — all p99 ≤ 190ms at c=100-150. admin/stats optimized 205→316 RPS (10 sequential count_documents → one conditional-sum aggregation + asyncio.gather). Indexes added: audit_logs (action,created_at), (actor,created_at), (details.ip,created_at); otp_challenges (ip,created_at).
  - EDGE: Cloudflare in front of every host returns 429 "Just a moment..." HTML to a single IP above ~200 req/s (platform DDoS protection; not the app, not reachable by real users).
  - CAVEAT for testers: admin lockout = 5 `admin_login_failed` per phone OR per IP in 15 min (unauthorized phones count) → clear audit_logs admin_login_failed test artefacts after hammering /api/admin/auth/send-otp.
  - Tests: tests/test_admin_login_e2e.py (seeded OTP challenge, no SMS; PASS on preview), tests/load_test.py (records failure bodies), pytest suite 27 pass.
- Phase 11 (2026-09-11) — MANAGE USERS (app staff: admin / telecaller / billing_executive) — COMPLETED (website side; app endpoints pending on the app team). USER DECISIONS: (1a) extend the app's integration-key API with staff endpoints — I write the spec, build against it + mock; (2a) ONE unified staff list: adding an admin here also grants website admin login (env ADMIN_PHONES stay as permanent bootstrap/fallback); (3a) roles exactly admin, telecaller, billing_executive; (4a) build now against the spec.
  - Our side: Mongo `staff_users` (id, name, phone, role, code, status, app_user_id, app_sync_status ok|pending|failed|not_configured, app_sync_error, source env|site|app, timestamps). Seeded from ADMIN_PHONES/TELECALLER_PHONES at startup. Admin login authorization = active staff_users ∪ env phones. Safety: cannot remove/demote yourself; cannot remove last active admin.
  - Admin API: GET/POST /api/admin/staff, PATCH/DELETE /api/admin/staff/{id}, POST /api/admin/staff/{id}/resync, POST /api/admin/staff/import-from-app, GET /api/admin/staff/app-status. Every change audited + pushed to app via integration key.
  - App contract (docs/YASH_TRADE_APP_INTEGRATION.md → "Staff" section): GET /api/integrations/staff[?role=&status=], POST /api/integrations/staff (upsert by phone), PATCH /api/integrations/staff/{id}, DELETE /api/integrations/staff/{id} (soft-disable; ?hard=true deletes). Header X-Integration-Key. 401/404/409(phone conflict)/422.
  - Frontend: nav "Manage Users" → /admin/staff (table, role tabs, search, add/edit dialogs, remove confirm, resync, import-from-app, app-endpoint status banner).
- Phase 12 (2026-09-11) — STAFF CONSOLE: Products (admin) · Queries (telecaller) · Rates (billing) — COMPLETED (website side, build 2026.09.11-console-v12; writes + Queries data unlock automatically once the app deploys Parts 2-3). USER DECISIONS: (1a) token-on-behalf: app adds `POST /api/integrations/staff/{phone}/token` (integration key → short-lived staff JWT); website acts AS the logged-in staff member against the app's own endpoints (permissions + attribution stay in the app). (2b) billing executive = metal rates + rate list. (3c) telecaller = requests/queries (status, notes, complete) + telecaller customers (call outcomes, follow-ups, activity, summary). (4a) product photos live in the app's storage (upload endpoint on app; default `/api/banners/upload`, env LIVE_PRODUCT_IMAGE_UPLOAD_PATH). (5a) same /admin/login, role-based workspace after OTP. Order: Products → Queries → Rates.
  - FACTS: app GET /api/products (343 items, filters page/limit/category/metal_type/search/post_type/include_hidden), /api/categories, /api/rates/latest, /api/rate-list are PUBLIC (no token) → product list + rates readable today. Images: `images[]` URLs or `/api/files/{thumbnail_path|storage_path}`. Writes need staff bearer token: products POST/PUT/DELETE, requests GET/PATCH/history, telecaller customers/action/activity/summary, rates POST, rate-list POST/PUT/DELETE.
  - Website backend: session gets live_token via integration token endpoint (background, bounded; legacy demo-OTP path only if app demo_mode true); `/api/portal/*` role-allowlisted thin proxies (admin: products+all; telecaller: requests+telecaller; billing: rates+rate-list); audit on every write. /admin/auth/me adds app_token_status.
  - Frontend: role-based AdminLayout nav + landing (admin → /admin, telecaller → /admin/queries, billing → /admin/rates); pages Products.js, Queries.js, Rates.js.
  - Mock: tests/mock_app_backend.py gains token endpoint + products/requests/telecaller/rates with bearer auth; contract tests.
  - 2026-09-11 (continuation) VERIFIED: /admin/products renders 343 live products with images (params forwarded: search/category/limit OK), /admin/rates shows live rates + rate list, /admin/queries shows graceful read-only state. Role UI verified by screenshot: telecaller → /admin/queries only, billing → /admin/rates only, admin page blocked for others with "Go to my workspace"; Manage Users add → row + toast + counters, remove → confirm dialog → gone. pytest 25 pass; testing agent iteration_7: backend 56/56 pass (RBAC, staff CRUD incl. self-demote/last-admin guards, graceful 503s, 422 validation). Real app (build 2026.09.09-integration-v7) still returns 404 for /api/integrations/staff* and the token endpoint → writes/queries stay "read-only until the app enables access" by design. USER DECISION: skip Play/App Store link swap for now; deliverables for the Trade App team: docs/YASH_TRADE_APP_INTEGRATION.md (status table + Part 3: token endpoint, role matrix, product photos, acceptance curl tests) + docs/PROMPT_FOR_TRADE_APP.md (copy-paste brief).
- NEXT (pending on others): app team deploys Parts 2-3 → verify with Manage Users → Re-check / Sync all, Products → Add photo, Queries → Mark completed. Still open from earlier: real Play/App Store URLs (ANDROID_APP_URL / IOS_APP_URL env or Admin Settings), admin SMS/WhatsApp alert on new enrollment (P2).
